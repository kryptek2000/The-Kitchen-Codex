/**
 * The Kitchen Codex — Advanced Nutrition Phase 0B: ONE effective mass-source
 * decision.
 *
 ...
 *     portion) is a CONFLICT and fails closed;
 ...
 *     any stored alternate choice (user mass, source portion, or count
 *     portion) is a CONFLICT and fails closed — it is never silently ignored,
 *     never displayed as active, and never persisted over the recipe mass.
 *     The UI does not offer alternate mass choices for such a line, and the
 *     core builders refuse to create them; this rule is the fail-closed
 *     backstop for forged, legacy, stale, or hand-built states.
 *
...
export function resolveEffectiveMassDecision(claims: EffectiveMassClaims): EffectiveMassDecision {
  const nonDirectCount =
    (claims.hasUserMass ? 1 : 0) + (claims.hasSourcePortion ? 1 : 0) + (claims.hasCountPortion ? 1 : 0);

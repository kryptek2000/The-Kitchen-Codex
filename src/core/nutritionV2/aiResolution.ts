 ...
 *    authority stays deterministic. Since v2/v3, local resolution additionally
 *    re-checks every accepted candidate against the AUTHENTICATED source line's
 *    negative container/state/form/variety constraints (reconstructed locally
 *    from the session-bound row), so provider wording can never erase a source
 *    constraint.

 ...
 *
 * PURE, offline, display-only. Recognized schema-v1/v2/v3 stored data is
 * validated before trusted display; malformed data is never shown as
 * authoritative; an unknown future schema is preserved opaquely and never
 * interpreted.
 *
...
export interface StoredAdvancedSummary {
  readonly kind: 'none' | 'v1' | 'v2' | 'v3' | 'opaque';
 ...
 * a recognized `schema: 1`/`schema: 2`/`schema: 3` / `basis: 'total'` block is
 * interpreted; everything else (opaque future schema or malformed data) is
 * reported as opaque. The value is materialized once into inert data before any
 * property is read.
 */
export function summarizeStoredAdvanced(blockRaw: unknown): StoredAdvancedSummary {
  ...
  if ((candidate.schema !== 1 && candidate.schema !== 2 && candidate.schema !== 3) || candidate.basis !== 'total') {
    return opaque();
  }
  ...
  return Object.freeze({
    kind: candidate.schema === 3 ? 'v3' : candidate.schema === 2 ? 'v2' : 'v1',
